...
def get_detail(url):
    ...
    try:
        ...
        add = "topilmadi"
    try:
        phone = json.loads(get_phone(url.split("/")[-1]))['data']['model']['phone']
    except:
        phone = "topilmadi"
    try:
...
def get_items(htmldom):
    ...
    return results
def moto_parser():
    print("Moto Parser ishga tushdi!")
    for num_page in range(1, 2):
        html = get_page(f"https://avtoelon.uz/uz/mototehnika/?page={num_page}")
        htmldom = BeautifulSoup(html.text, "lxml")
    

        items = get_items(htmldom)
        with open(f"page_Moto.json", "w", encoding="UTF8") as file:
